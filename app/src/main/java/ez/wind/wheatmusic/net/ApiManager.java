package ez.wind.wheatmusic.net;
...
import ez.wind.wheatmusic.BuildConfig;
import ez.wind.wheatmusic.R;
import ez.wind.wheatmusic.app.WheatApp;
import ez.wind.wheatmusic.net.api.gson.MyGsonConverterFactory;
...
public class ApiManager {
    ...
    private static OkHttpClient getOkHttpClient() {
        if (mOkHttpClient == null) {
            ...
            ClearableCookieJar cookieJar =
                    new PersistentCookieJar(new SetCookieCache(), new SharedPrefsCookiePersistor(WheatApp.Companion.getInstance().getApplicationContext()));
            synchronized (ApiManager.class) {
                Cache cache = new Cache(new File(WheatApp.Companion.getInstance().getApplicationContext().getCacheDir(), "HttpCache"), 1024 * 1024 * 100);
                if (mOkHttpClient == null) {
    ...
    public static <T> void request(Observable<T> service, RequestCallBack<T> result) {
        if (!NetworkUtils.isNetworkAvailable(WheatApp.Companion.getInstance().getApplicationContext())) {
            result.error(WheatApp.Companion.getInstance().getApplicationContext().getString(R.string.error_connection));
            return;
        }
